...
class ActivityTracker:
                ...
                for app in self.app_manager.applications:
                    ...
                    if is_running:
                        ...
                        if app_name not in self.running_apps:
                            ...
                            print(f"[Tracker] Start detected: {app_name}")
                    else:

                        if app_name in self.running_apps:
                            self._record_session_end(app_name)
                            print(f"[Tracker] Stop detected: {app_name}")
    ...
    def _record_session_start(self, app_name, process):
        ...
        print(f"[Tracker] Session started for {app_name} at {now.strftime('%H:%M:%S')}")
    ...
    def _record_session_end(self, app_name):
        ...
        print(f"[Tracker] Session ended for {app_name}: {duration_str}")
    ...
    def remove_app_data(self, app_name):
        ...
        if app_name in self.activities:
            ...
            self.save_activities()
            print(f"[Tracker] Data deleted for: {app_name}")
            return True
        return False
    ...
    def print_status(self):
        ...
        print(f"  Running applications: {', '.join(status['running_apps']) if status['running_apps'] else 'None'}")
        print(f"  Total apps tracked: {status['total_apps_tracked']}")
...
class GoalsManager:
    ...
    def add_goal(self, app_name, goal_type, limit_value, period="daily"):
        ...
        Args:
            app_name: Name of the application (or category)
            goal_type: 'max_time' (limit time) or 'min_time' (encourage usage)
            limit_value: Limit in seconds
    ...
    def remove_goals_for_app(self, app_name):
        ...
        for goal_id in goals_to_remove:
            del self.goals[goal_id]
            print(f"[Goals] Goal deleted: {goal_id}")
                ...
                if current_time >= limit_value:
                    ...
                    if notification_key not in self.notifications_shown:
                        alert = {
                            ...
                            "message": f"⚠️ Limit exceeded for {app_name}\n"
                                     f"Time used: {self._format_time(current_time)}\n"
                                     f"Limit: {self._format_time(limit_value)}"
                        }
                    ...
                    if warning_key not in self.notifications_shown:
                        alert = {
                            ...
                            "limit_value": limit_value,
                            "percentage": percentage,
                            "message": f"⚡ Warning: {app_name}\n"
                                     f"You have used {percentage:.0f}% of your daily limit\n"
                                     f"Time remaining: {self._format_time(limit_value - current_time)}"
                        }
    ...
    def get_pinned_goals(self, hide_completed=False):
        """Returns the list of pinned goals
        
        Args:
            hide_completed: If True, hides achieved goals (100% or more)
        """
        pinned = []
        for goal_id, goal in self.goals.items():
...
class XClientApp:
    def __init__(self, root):
        ...
        add_placeholder(self.search_entry, "Search...")
        ...
        ToolTip(self.stats_button, "View usage statistics")
    ...
    def open_settings_menu(self):
        ...
        def show_shortcuts():
            ...
            shortcuts = [
                ("Ctrl + G", "Open groups interface"),
                ("Ctrl + A", "Add an application"),
                ("Ctrl + O", "Manage goals"),
        ...
        create_menu_button("⌨️ View keyboard shortcuts", show_shortcuts)
        ...
        create_menu_button("🔄 Update XClient", 
                         lambda: os.startfile(os.path.join("update", "XClient_installation")))


        create_menu_button("🔗 View on GitHub",
    ...
    def open_activity_dashboard(self):
        ...
        self.setup_window(win, "Activity Dashboard")
        

        header = tk.Frame(win, bg="#1e2124")
        header.pack(fill="x", padx=20, pady=(20, 10))
        
        tk.Label(header, text="Application Usage Statistics", 
        ...
        def update_stats(period_str=None):
            ...
            if period_str == "Today":
                days = 1
            elif period_str == "7 days":
                days = 7
            elif period_str == "30 days":
                days = 30
            else:
                days = 36500
            ...
            if not stats:
                tk.Label(scrollbar_frame, text="No activity data available", 
            ...
            tk.Label(right_graph, text="Top 10 - Usage Time (hours)", 
                    bg="#2e3440", fg="white", font=("Arial", 12, "bold")).pack(pady=10)
            ...
            if times_hours:
                ...
                ax_bar.set_xlabel('Hours', color='white')
                ax_bar.invert_yaxis()
            ...
            tk.Label(table_frame, text="Application Details", 
                    bg="#2e3440", fg="white", font=("Arial", 12, "bold")).pack(pady=10)
            ...
            tk.Label(headers_frame, text="Total time", width=12, anchor="center",
                    bg="#1e2124", fg="white", font=("Arial", 9, "bold")).pack(side="left", padx=5)
            tk.Label(headers_frame, text="Launches", width=12, anchor="center",
                    bg="#1e2124", fg="white", font=("Arial", 9, "bold")).pack(side="left", padx=5)
            tk.Label(headers_frame, text="Average time", width=12, anchor="center",
                    bg="#1e2124", fg="white", font=("Arial", 9, "bold")).pack(side="left", padx=5)
            tk.Label(headers_frame, text="Last used", width=18, anchor="center",
                    bg="#1e2124", fg="white", font=("Arial", 9, "bold")).pack(side="left", padx=5)
    ...
    def open_goals_manager(self, parent_window=None):
        ...
        self.setup_window(win, "Goal Management")
        ...
        if not self.goals_manager.goals:
            ...
            for goal_id, goal in self.goals_manager.goals.items():
                ...
                toggle_text = "✓ Enabled" if goal.get("enabled", True) else "✗ Disabled"
                ...
                goal_type_text = "Maximum limit" if goal["goal_type"] == "max_time" else "Minimum goal"
                period_text = {"daily": "per day", "weekly": "per week", "monthly": "per month"}.get(goal["period"], "per day")
    ...
    def open_add_goal_dialog(self, parent_window):
        ...
        tk.Label(form, text="Application or Category", bg="#1e2124", fg="white",
        ...
        tk.Label(form, text="Goal Type", bg="#1e2124", fg="white",
                font=("Arial", 10, "bold")).pack(anchor="w", pady=(0, 5))
        ...
        tk.Radiobutton(type_frame, text="⏱️ Limit usage time", variable=goal_type_var,
                      value="max_time", bg="#1e2124", fg="white", selectcolor="#2e3440",
                      activebackground="#1e2124", activeforeground="white",
                      font=("Arial", 10)).pack(anchor="w", pady=2)
        
        tk.Radiobutton(type_frame, text="🎯 Encourage usage (minimum)", variable=goal_type_var,
                      value="min_time", bg="#1e2124", fg="white", selectcolor="#2e3440",
                      activebackground="#1e2124", activeforeground="white",
                      font=("Arial", 10)).pack(anchor="w", pady=2)
        

        tk.Label(form, text="Duration (hours and minutes)", bg="#1e2124", fg="white",
                font=("Arial", 10, "bold")).pack(anchor="w", pady=(0, 5))
        
        time_frame = tk.Frame(form, bg="#1e2124")
        time_frame.pack(fill="x", pady=(0, 15))
        
        tk.Label(time_frame, text="Hours:", bg="#1e2124", fg="white").pack(side="left", padx=(0, 5))
        ...
        tk.Label(form, text="Period", bg="#1e2124", fg="white",
                font=("Arial", 10, "bold")).pack(anchor="w", pady=(0, 5))
        ...
        def save_goal():
            ...
            if not app_name:
                messagebox.showwarning("Error", "Please select an application or category")
                return
    ...
    def open_icon_picker(self, icon_entry):
        ...
        tk.Label(header, text="Choose an XClient icon", 
                font=("Arial", 14, "bold"), fg="white", bg="#1e2124").pack(side="left")
        ...
        add_placeholder(search_entry, "Search...")
        ...
        print(f"[Icon Picker] {len(icon_files)} icons found in '{icon_folder}/'")
        
        if not icon_files:
            tk.Label(icons_frame, text="No icons found in the icon/ folder",
            ...
            def filter_icons(*args):
                ...
                if displayed == 0:
                    tk.Label(icons_frame, text="No icons match your search",
                            fg="#9aa0a6", bg="#1e2124", font=("Arial", 11)).pack(pady=50)
        ...
        apply_btn = RoundedButton(buttons_frame, width=180, height=32, cornerradius=8,
                                 bg="#f84444", fg="white", text="Use this icon",
    ...
    def open_groups_manager(self):
        ...
        self.setup_window(win, "Group Management")
        ...
        auto_cat_check = tk.Checkbutton(options_frame, text="Automatic categorization",
        ...
        tk.Label(list_frame, text="Groups", bg="#1e2124", fg="white", font=("Arial", 11, "bold")).pack(anchor="w", pady=(0,8))
        ...
        tk.Label(detail, text="Group name", bg="#1e2124", fg="white", 
        ...
        tk.Label(detail, text="Icon (optional)", bg="#1e2124", fg="white", 
                font=("Arial", 10, "bold")).pack(anchor="w", pady=(0, 8))
        ...
        ToolTip(group_browse_icon_btn, "Select a local image file")
    ...
    def add_application(self):
        def save_app():
            ...
            if exe_path.lower().endswith('.url'):
                try:
                    ...
                    messagebox.showerror("Error", f"Error copying the .url file: {str(e)}")
                    return
                ...
                try:
                    ...
                    messagebox.showerror("Error", f"Error copying the file: {str(e)}")
                    return
        ...
        def detect_and_update_category():
            ...
            if exe_path and self.auto_categorize:
                ...
                if detected_category != DEFAULT_GROUP_ID and detected_category in self.groups:
                    ...
                    category_label.config(text=f"✓ Category detected: {self.groups[detected_category]['name']}", 
                                        fg="#4caf50")
                else:
        ...
        title_label = tk.Label(title_frame, text="Add a new application",
                             font=("Arial", 16, "bold"), fg="white", bg="#1e2124")
        title_label.pack()
        ...
        tk.Label(form_frame, text="Application name", bg="#1e2124", fg="white",
                font=("Arial", 10, "bold")).pack(anchor="w", pady=(0, 8))
        ...
        name_entry = tk.Entry(name_entry_frame, font=("Arial", 11),
                            ...
                            highlightcolor="#4a90e2")
        name_entry.pack(fill="both", expand=True, padx=2, pady=2, ipady=8)
        add_placeholder(name_entry, "Readable name (e.g., Google Chrome)")


        tk.Label(form_frame, text="Executable path", bg="#1e2124", fg="white",
                font=("Arial", 10, "bold")).pack(anchor="w", pady=(0, 8))
        ...
        add_placeholder(exe_entry, "Path to executable or .url")
        ...
        exe_btn = RoundedButton(form_frame, width=150, height=32, cornerradius=8,
                               ...
                               command=lambda: exe_entry.delete(0, tk.END) or exe_entry.insert(0, filedialog.askopenfilename(
                                   filetypes=[
                                       ("Executable files", "*.exe;*.msi;*.bat;*.cmd;*.vbs;*.ps1;*.reg;*.dll;*.appref-ms;*.url"),
                                       ("All files", "*.*"),
        ...
        ToolTip(exe_btn, "Select an executable file")


        tk.Label(form_frame, text="Icon (URL or file, Optional)", bg="#1e2124", fg="white",
                font=("Arial", 10, "bold")).pack(anchor="w", pady=(0, 8))
        ...
        icon_entry = tk.Entry(icon_entry_frame, font=("Arial", 11),
                            ...
                            highlightcolor="#4a90e2")
        icon_entry.pack(fill="both", expand=True, padx=2, pady=2, ipady=8)
        add_placeholder(icon_entry, "Image file or URL (optional)")
        ...
        ToolTip(browse_icon_btn, "Select a local image file")
        
        xclient_icon_btn = RoundedButton(icon_buttons_frame, width=180, height=32, cornerradius=8,
                                        bg="#4a90e2", fg="white", text="🎨 XClient Icons",
                                        command=lambda: self.open_icon_picker(icon_entry))
        xclient_icon_btn.pack(side="left")
        ToolTip(xclient_icon_btn, "Choose from XClient icons")


        tk.Label(form_frame, text="Group", bg="#1e2124", fg="white",
                font=("Arial", 10, "bold")).pack(anchor="w", pady=(0, 8))
        ...
        add_btn = RoundedButton(buttons_frame, width=200, height=28, cornerradius=8,
                              bg="#f84444", fg="white", text="Add application",
                              command=save_app)
    ...
    def modify_application(self, index):
        ...
        self.setup_window(modify_window, "Modify Application")
        ...
        ToolTip(new_exe_btn, "Select an executable file")
        ...
        ToolTip(new_browse_icon_btn, "Select a local image file")
        ...
        ToolTip(new_xclient_icon_btn, "Choose from XClient icons")
        ...
        save_btn = RoundedButton(buttons_frame, width=200, height=28, cornerradius=8,
                               bg="#f84444", fg="white", text="Save changes",
    ...
    def delete_application(self, index):
        ...
        dialog.geometry("600x500")
        self.setup_window(dialog, "Confirm Deletion")
        ...
        stats_check = tk.Checkbutton(options_frame, 
                                     text="Also delete all statistics and goals related to this application",
        ...
        info_label = tk.Label(options_frame, 
                             text="⚠ If you keep the statistics, they will remain visible\nin the dashboard even after deletion.",
        ...
        delete_btn = RoundedButton(button_frame, width=220, height=35, cornerradius=6,
                                 bg="#ff4444", fg="white", text="Delete",
        ...
        try:
            if 0 <= index < len(self.applications):
                ...
                if delete_stats:
                    ...
                    stats_msg = []
                    if removed_activity:
                        stats_msg.append("usage statistics")
                    if removed_goals > 0:
                        stats_msg.append(f"{removed_goals} goal(s)")
                    
                    if stats_msg:
                        messagebox.showinfo("Deletion successful", 
                                          f'Application "{app_name}" deleted.\n\n'
                                          f'Also deleted: {", ".join(stats_msg)}.')
    ...
    def update_app_grid(self):
        ...
        if not apps:
            msg = "No applications" if not self.search_query else "No applications match your search"
        ...
        for app in apps:
            ...
            try:
                ...
                print(f"Error loading action icons: {e}")
                edit_photo = None
                delete_photo = None
            ...
            delete_btn = tk.Button(button_frame, image=delete_photo if delete_photo else "×",
                                ...
                                cursor="hand2", width=20 if not delete_photo else None,
                                height=20 if not delete_photo else None,
                                highlightthickness=0,
                                activebackground="#2e3440",
                                borderwidth=0)
    ...
    def detect_app_category(self, exe_path, app_name=""):
        """
        Automatically detects the category of an application
        ...
        Returns:
            str: ID of the detected group or DEFAULT_GROUP_ID
        """
        if not exe_path:
    ...
    def recategorize_all_apps(self):
        ...
        if recategorized_count > 0:
            self.save_all()
            self.update_app_grid()
            messagebox.showinfo("Categorization", 
                              f"{recategorized_count} application(s) have been automatically recategorized.")
        else:
            messagebox.showinfo("Categorization", 
                              "No applications to recategorize.")
    ...
    def cleanup_orphaned_stats(self):
        ...
        if not orphaned_stats and not orphaned_goals:
            messagebox.showinfo("Cleanup", "No orphaned statistics found.")
            return
        

        message = "Applications with statistics but not configured:\n\n"
        if orphaned_stats:
            message += f"Statistics: {len(orphaned_stats)} app(s)\n"
            message += "   - " + "\n   - ".join(list(orphaned_stats)[:5])
            if len(orphaned_stats) > 5:
                message += f"\n   ... and {len(orphaned_stats) - 5} others\n"
        if orphaned_goals:
            message += f"\n🎯 Goals: {len(orphaned_goals)} app(s)\n"
        
        message += "\n\nDo you want to delete this data?"
        
        if messagebox.askyesno("Cleanup statistics", message):
            removed_count = 0
            ...
            messagebox.showinfo("Cleanup complete", 
                              f"{removed_count} item(s) deleted.")
    ...
    def _build_sidebar(self):
        ...
        if settings_icon:
            ...
            ToolTip(mg, "Open group manager")
        
        title_grp = tk.Label(self.sidebar, text="Groups", bg="#161a1e", fg="#9aa0a6", font=("Arial", 10, "bold"))
    ...
    def _rename_selected_group(self, name_entry):
        ...
        gid = self._group_id_from_listbox_index(sel[0])
        if gid == DEFAULT_GROUP_ID:
            messagebox.showinfo("Info", "The default group cannot be renamed.")
            return
    ...
    def _update_goals_progress_display(self):
        ...
        tk.Label(header, text="Active Goals", font=("Arial", 11, "bold"),
        ...
        hide_tooltip = "Show completed goals" if self.hide_completed_goals else "Hide completed goals"
        ...
        try:
            ...
            if hasattr(self, 'system_tray_icon') and self.system_tray_icon:
                ...
                if alert_type == "limit_exceeded":
                    title = "⚠️ XClient - Limit Exceeded"
                elif alert_type == "approaching_limit":
                    title = "⚡ XClient - Warning"
                elif alert_type == "goal_achieved":
                    title = "✅ XClient - Goal Achieved"
                else:
    ...
    def _show_goal_popup(self, alert):
        ...
        if alert["type"] in ["limit_exceeded", "approaching_limit"]:
            ...
            tk.Label(message_frame, text=f"Time used: {time_current} / {time_limit}", 
            ...
            tk.Label(message_frame, text=f"Time used: {time_str}", 
                    font=("Arial", 11), bg=bg_color, fg="white").pack(anchor="w", pady=(5, 0))
            tk.Label(message_frame, text="Congratulations! Keep it up! 🎉", 
